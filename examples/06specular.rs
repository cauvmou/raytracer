use raytracer::{camera::{Camera, Screen}, surface::Scene, meshes::{Plane, Sphere}, materials::{SDMaterial}, light::{SceneLights, DirectionalLight, PointLight}};
...
fn main() {
    ...
    scene.push(Box::new(Plane::new(
        Box::new(SDMaterial::new([0, 0, 255].into(), 0.5, 0.2, 10.0)),
        (0.0, 0.0, 0.0).into(),
        (0.0, 0.0, 1.0).into(),
    )));

    scene.push(Box::new(Sphere::new(
        Box::new(SDMaterial::new([255, 0, 0].into(), 0.6, 0.1, 5.0)),
        (0.0, 0.0, 4.0).into(),
        3.0,
    )));

    scene.push(Box::new(Sphere::new(
        Box::new(SDMaterial::new([0, 255, 0].into(), 0.6, 0.1, 5.0)),
        (-5.0, 10.0, 6.0).into(),
        3.0,
    )));
    ...
    screen.export("./out/06out.png").expect("Failed to save image.");
}
